...
def run_rule_checks(engine, ocr_data):
    ...
    if y_values:
        for check, key in [
            ("truncated_axis", "is_truncated"),
            ("broken_scale", "is_broken"),
            ("inverted_axis", "is_inverted"),
        ]:
            try:
                r = engine.run_check(
                    check, {"axis_values": y_values, "chart_type": chart_type}
                )
                if r.get(key):
...
def build_ocr_summary(ocr_data):
    ...
    for key, label in [
        ...
        ("right_y_axis_label", "Right Y-axis"),
...
def main():
    ...
    def process_one(chart):
        ...
        try:
            ...
            raw_judge = vlm_call(
                client, model, image_b64, PIPELINE_SYSTEM_PROMPT, judge_prompt
            )
        ...
        with lock:
            ...
            print(
                f"  LLM-OCR+Rules: misleaders={types2} completeness={len(compl)} rules={rules}"
            )
    ...
    (OUT_DIR / "vlm_only.json").write_text(
        json.dumps(results_vlm, indent=2, default=str), encoding="utf-8"
    )
    (OUT_DIR / "llm_ocr_rules.json").write_text(
        json.dumps(results_ocr, indent=2, default=str), encoding="utf-8"
    )
    ...
    vlm_flagged = sum(
        1 for r in results_vlm if r.get("misleading") and "error" not in r
    )
    vlm_total = sum(1 for r in results_vlm if "error" not in r)
    ocr_flagged = sum(
        1 for r in results_ocr if r.get("misleaders") and "error" not in r
    )
    ocr_total = sum(1 for r in results_ocr if "error" not in r)

    print(
        f"VLM-only: {vlm_flagged}/{vlm_total} flagged ({vlm_flagged/max(vlm_total,1)*100:.0f}%)"
    )
    print(
        f"LLM-OCR+Rules: {ocr_flagged}/{ocr_total} flagged ({ocr_flagged/max(ocr_total,1)*100:.0f}%)"
    )
    ...
    for r in results_vlm:
        if "error" not in r:
            ...
            if r.get("misleading"):
                ticker_vlm[r["ticker"]]["flagged"] += 1
    for r in results_ocr:
        if "error" not in r:
            ticker_ocr[r["ticker"]]["total"] += 1
            if r.get("misleaders"):
                ticker_ocr[r["ticker"]]["flagged"] += 1
