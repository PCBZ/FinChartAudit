...
class SECDownloader:
    ...
    def get_filings(
        self, cik: str, filing_type: str = "10-K", count: int = 10
    ) -> list[dict]:
    ...
    def download_company_data(
        self, ticker: str, max_10k: int = 3, max_comments: int = 10
    ):
        ...
        if comments:
            ...
            for i, c in enumerate(comments):
                print(
                    f"  {i+1}. {c['filingDate']} | {c['form']:<8} | {c['accessionNumber']}"
                )
        else:
...
def download_sec_filings(
    submissions_url: str = None, archives_url: str = None, user_agent: str = None
):
    ...
    for ticker in SECDownloader.COMPANIES:
        ...
        for filing in meta.get("filings_10k", []):
            ...
            if content:
                ...
                print(
                    f"  ✓ {ticker} {date} → {out_path.name} ({len(content)//1024} KB)"
                )
