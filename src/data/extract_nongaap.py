...
def clean_text(raw: bytes) -> str:
    ...
    text = re.sub(
        r"(?s).*?(?=Dear\s|Re:\s|Ladies|Gentlemen|We\s(are|have|write)|This\s(letter|response))",
        "",
        text,
        count=1,
        flags=re.IGNORECASE,
...
def extract_nongaap_mentions(
    sentences: list[str], context: int = CONTEXT_WINDOW
) -> list[dict]:
    """
...
def process_letters(
    letters_dir: str = "data/letters", output_path: str = "data/ground_truth.json"
):
    ...
    total_mentions = sum(
        e["total_nongaap_mentions"] for v in results.values() for e in v
    )
    print(
        f"📊 {total_files} files with Non-GAAP mentions, "
