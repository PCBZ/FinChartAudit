...
def run_single(client: OpenAI, model: str, image_url: str, condition: str, ground_truth_text: str = "") -> dict:
    """Run inference on a single sample."""
    prompt = build_vision_only_prompt() if condition == "vision_only" else build_vision_text_prompt(ground_truth_text)
...
def evaluate(api_key: str, model_key: str, condition: str):
    ...
    print(f"Model: {model} | Condition: {condition} | Samples: {len(samples)}")
    ...
    for i, item in enumerate(samples):
        ...
        pred = run_single(client=client, model=model, image_url=f"data:image/png;base64,{img_b64}", condition=condition)

        # Metrics
        gt_labels   = set(sample['gt_misleaders'])
        pred_labels = set(pred.get('misleader_types', []))

        # Binary: misleading or not
        ...
        if (i + 1) % 50 == 0:
            print(f"  [{i+1}/{len(samples)}] binary acc so far: {correct/total:.3f}")
...
if __name__ == "__main__":
    ...
    load_dotenv(Path(__file__).resolve().parents[1] / ".venv" / ".env")
    
    evaluate(api_key=os.environ["OPENROUTER_API_KEY"], model_key="claude", condition="vision_only")